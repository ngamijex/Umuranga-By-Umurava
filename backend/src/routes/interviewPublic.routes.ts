...
/* POST /public/interview/:token/complete — mark interview as completed when user ends the call early */
router.post("/:token/complete", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Async grading if there is a transcript (non-blocking)
    if (session.transcript.length > 1 && !session.score) {
      const [job, candidateDoc] = await Promise.all([
        Job.findById(session.jobId),
        Candidate.findById(session.candidateId).lean(),
      ]);
      if (job) {
        const { gradeInterview } = await import("../services/interview.service");
        const profile = candidateDoc ? {
          name: `${(candidateDoc as any).firstName || ""} ${(candidateDoc as any).lastName || ""}`.trim(),
          skills: (candidateDoc as any).skills || [],
          experience: (candidateDoc as any).experience || [],
          education: (candidateDoc as any).education || [],
        } : undefined;
        gradeInterview(job, session.transcript, profile as any)
          .then(score => InterviewSession.findByIdAndUpdate(session._id, { score }))
          .catch(e => console.error("[interview] grading on manual end:", e?.message));
      }
    }
